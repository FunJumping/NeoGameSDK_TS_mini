    ...
    export class PayTransferView extends ViewBase {
        ...
        static get_log_type_detail_by_asset(asset: string): string {
            for (let k in PayTransferView.log_type_detail) {
                ...
                if (Main.netMgr.getCoinTool().hasOwnProperty(id) && Main.netMgr.getCoinTool()[id] == asset) {
        ...
        create() {
            ...
            this.selectType.onchange = () => {
                ...
                if (!Main.in_array(this.transferType, ["BCT", "BCP"])) {
        ...
        private async doTransfer() {
            ...
            try {
                switch (coin_type) {
                    case 'GAS_2': // gas转账 1
                    case "NEO_2": // neo转账 2
                        var res: Result = await tools.CoinToolNeo.rawTransaction(this.toaddress, tools.CoinToolNeo["id_" + this.transferType], this.inputTransferCount.value, Neo.Fixed8.fromNumber(Number(net_fee)));
                        break;
                    case 'CGAS_2': // CGAS 3
                    case "CNEO_2": // CNEO 4
                        var res: Result = await tools.CoinToolNeo.nep5Transaction(Main.user.info.wallet, this.toaddress, tools.CoinToolNeo["id_" + this.transferType], this.inputTransferCount.value, net_fee);
                        break;
                }
            }
            catch (e) {
            ...
            if (res) {
                ...
                if (res.err == false) {
                    // 成功，记录
                    ApiTool.addUserWalletLogs(
                        res.info,
                        this.inputTransferCount.value,
                        "6",
                        '{"sbPushString":"transfer", "toaddr":"' + this.toaddress + '", "count": "' + this.inputTransferCount.value + '", "nnc": "' + Main.netMgr.getCoinTool()["id_" + this.transferType] + '"}',
                        net_fee,
                    ...
                    // 转账失败
                    if (Main.in_array(res.info, ["get_gasLimit_fail", "gasLimit_not_enough"])) {
                        let err = Main.langMgr.get(res.info, {gasLimit: res['gas_consumed']})
                        Main.showErrMsg("pay_transferDoFailErr", null, {err: err})
                    }
                    else {
        ...
        private getBalanceTypes() {
            ...
            Main.netMgr.getChainCoins(chain).forEach((coin) => {
                // if (!Main.in_array(token, ["btc", "eth"])) {
