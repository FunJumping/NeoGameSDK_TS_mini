...
namespace BlackCat.tools
{
    ...
    export class NNSTool
    {
        ...
        static async registerDomain(doamin: string)
        {
            ...
            var data = sb.ToArray();
            var res = await tools.CoinToolNeo.contractInvokeTrans_attributes(data);
            if (!res.err)
            {
        ...
        static async getRootName(): Promise<string>
        {
            ...
            let result = await tools.WWWNeo.api_getInvokescript(data);
            try
            {
        ...
        static async setResolve(nnshash: Uint8Array, resolverhash: Uint8Array): Promise<Result>
        {
            ...
            let res = await tools.CoinToolNeo.contractInvokeTrans_attributes(data);
            return res;
        }
        ...
        static async resolveData(domain: string)
        {
            ...
            sb.EmitAppCall(scriptaddress);
            var data = sb.ToArray();
            let res = await tools.WWWNeo.api_getInvokescript(data);
            let addr = "";
